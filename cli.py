...
@click.option("--temperature", default=0.6, help="Sampling temperature (higher = more random).")
...
def generate(prompt, num_tokens, temperature, top_k):
    ...
    out = sample(model, idx, num_tokens=num_tokens, stoi=stoi, itos=itos, temperature=temperature, top_k=top_k, device=device)
